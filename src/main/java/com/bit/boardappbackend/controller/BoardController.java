    ...
    public ResponseEntity<?> modify(@RequestPart("boardDto") BoardDto boardDto,
        ...
        try{
            ...
            if(uploadFiles != null && uploadFiles.length > 0)
                Arrays.stream(uploadFiles).forEach(file -> log.info("modify uploadFile: {}", file.getOriginalFilename()));

            if(changeFiles != null && uploadFiles.length > 0)
                Arrays.stream(changeFiles).forEach(file -> log.info("modify changeFile: {}", file.getOriginalFilename()));
